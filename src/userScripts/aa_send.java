  ...
  protected void doGet(
    ...
    try {
      ...
      try {
        ...
        rs =
          st.executeQuery("select * from aalist where AID = '" + user_id + "'");
        while (rs.next()) {
      ...
      try {
        ...
        if (rs.next()) {
          ...
          int i = st.executeUpdate(
            "update " + aaRTable + " set status = 'Yes' where id = '" + id + "'"
          );
          if (i == 0) {
          ...
          rs =
            stmt.executeQuery(
              "select audd from reg where name = '" + name + "'"
            );
          while (rs.next()) {
          ...
          rs =
            stmt.executeQuery(
              "select subShare from " + aaSTable + " where UID = '" + uid + "'"
            );
          while (rs.next()) {
          ...
          i =
            st.executeUpdate(
              "update request set " +
              aaArr[Integer.parseInt(aaNum) - 1] +
              " = '" +
              subShare +
              "' where id = '" +
              id +
          ...
          if (currCount >= t) {
            ...
            String emailtemp =
              ...
              name +
              ",</span></p><p><span style=\"font-size:14px;\">File request for <u>" +
              fname +
              "</u> has been&#xA0;approved! </span><br><span style=\"font-size:14px;\">Visit sunnydhama to download!</span></p><p><span style=\"font-size:14px;\">Regards,<br>Cloud Admin - sunnydhama</span><br>(This is system generated mail and should not be replied to)</p></div></td></tr></tbody></table></div><!--[if mso | IE]>      </td></tr></table>      <![endif]--></td></tr></tbody></table></div><!--[if mso | IE]>      </td></tr></table>      <![endif]--></div></body></html>";
            ...
            i =
              st.executeUpdate(
                "update request set status= 'Yes' where id = '" + id + "'"
              );
            con.close();
            st.close();
            if (i != 0) {
