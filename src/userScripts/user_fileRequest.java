...
public class user_fileRequest extends HttpServlet {
  ...
  protected void doGet(
    ...
    try {
      ...
      rs =
        stmt.executeQuery("select * from file_upload where id = '" + id + "'");
      if (rs.next()) {
        ...
        String Status = "No";
        System.out.println(
          "Request is activated" + Cname + State + State + Country + Status
        );
        ...
        sql =
          "insert into request (name, mail, state, country, fname, status) values ('" +
          Cname +
          "','" +
          Cmail +
          "','" +
        ...
        for (int i = 0; i < n; i++) {
          try {
            ...
            sql =
              "insert into " +
              aaArr[i] +
              " (id, name, mail, state, country, fname, status) values ('" +
              colCount +
              "','" +
              Cname +
              "','" +
